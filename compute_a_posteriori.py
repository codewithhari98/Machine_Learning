...
def __main__(argv):
    if argv[1]:
        ...
        if pattern.match(sequence):
            ...
            prob_cherry = 0
            for i in range(0,5):
                prob_lime = prob_lime + (prob_lime_h[i]*p_h[i])
            ...
            for j in range(0, len(sequence)):
                ...
                f.write('\nProbability that the next candy we pick will be L, given Q: ' + str(round(prob_lime, 5)))

        else:
            exit('Sequence should have series of L or C')
    else:
        exit("Follow the syntax: \npython compute_a_posteriori.py [sequence]")

p_h = [0.10, 0.20, 0.40, 0.20, 0.10]
p_c_h = [1.00, 0.75, 0.50, 0.25, 0.00]
prob_lime_h = [0.00, 0.25, 0.50, 0.75, 1.00]
__main__(sys.argv)cd 